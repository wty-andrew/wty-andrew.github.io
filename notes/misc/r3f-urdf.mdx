...
import { StaticModel, DynamicModel, Viewer } from '../../src/diagrams/robot-viewer'
...
[URDF][1] is the abbreviation of Unified Robot Description Format, which is an XML format for representing a robot model in ROS, we can display a model like above in browser with [react three fiber][2][^1] through [urdf-loaders][3].
...
```xml my_pkg/urdf/foo.urdf mark=3
...
<geometry>
